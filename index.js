...
if(!bUser) return msg.channel.send("استخدم: `[ ${p}باند @user <السبب> ] مثال : ${p} @الشخص نشر`");
...
client.on('message', message => {
    if(message.content.includes('discord.gg')){
                                            if(!message.channel.guild) return message.reply('** انت غبي؟ **');
...
const channel = rWlc[message.guild.id].role
  if (message.content.startsWith(prefix + "رتبه التلقائيه")) {
    if(!message.member.hasPermission(`MANAGE_GUILD`)) return;
    let newrole = message.content.split(' ').slice(1).join(" ")
    if(!newrole) return message.reply(`**${prefix}رتبه التلقائيه <rule name>**`)
...
m.sendMessage(args)
  ...
  client.on('message', msg => {
    if(msg.content === '#help')
    msg.reply('شف خاصك :white_check_mark:')
  });
...
client.on('message', async message =>{
  ...
  if (!message.content.startsWith(prefix)) return;
  if(!message.channel.guild) return message.channel.send('**الامر للسيرفرات**').then(m => m.delete(5000));
  ...
  var args = message.content.split(" ").slice(1);
    if(command == "mute") {
      ...
      if(!tomute) return message.reply(":information_source: `#اسكت @OrochiX` يجب تحديد شخص ") .then(m => m.delete(5000));
   ...
   client.on('message', async message => {
      ...
      if(!message.guild.member(client.user).hasPermission("MUTE_MEMBERS")) return message.reply("**I donot Have Permission Mute_Members**").then(msg => msg.delete(6000))
        ...
        let kinggamer = message.guild.member(message.mentions.users.first()) || message.guild.members.get(args[0]);
           if(!kinggamer) return message.channel.send(':information_source:  `#تكلم @OrochiX` يجب تحديد شخص ').then(msg => {
            msg.delete(3500);
            message.delete(3500); 
          });
        ...
        message.channel.sendMessage(`**:white_check_mark: ${mention.user.username}  تم فك ميوت **`);      
        return;
...
const channel = sWlc[message.guild.id].channel
  if (message.content.startsWith(prefix + "عمل ترحيب")) {
    if(!message.member.hasPermission(`MANAGE_GUILD`)) return;
    let newChannel = message.content.split(' ').slice(1).join(" ")
    if(!newChannel) return message.reply(`**${prefix}عمل ترحيب <channel name>**`)
...
client.on("message", async function(msg) {
 ...
 client.on('guildMemberAdd', member => {
    ...
    guild.channels.find(channel => channel.name === "chat").send("اهلا بك في سيرفرنا "+member.user.username);
});
